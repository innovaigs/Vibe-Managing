...
description: Standardize what must be common while preserving local responsiveness across sites, branches, stores, or franchises. Use when a founder or agent needs an evidence-backed multi-location operations decision or operating plan.
metadata:
  domain: operations
  version: 1.0.0
  autonomy_ceiling: L2
  provenance: CLAUDE
  reads: "company, goals, strategy, metrics, decisions, operations"
  writes: "operations, metrics, decisions, strategy"
  related_skills: "process-mapping, bottleneck-analysis, quality-management-system"
  owned_by_agents: "operations-agent, adaptation-agent, business-analyst-agent"
---

# Skill: Multi-location Operations

## Purpose
Standardize what must be common while preserving local responsiveness across sites, branches, stores, or franchises. It operationalizes capacity, flow, reliability, service delivery, and multi-site execution for companies of different sizes, models, industries, stages, and jurisdictions.

## When to Use
- The founder asks for a multi-location operations diagnosis, decision, plan, or review.
...
## Analysis Framework
1. **Define operating standards.** Produce evidence, confidence, and a decision implication before continuing.
2. **benchmark locations.** Produce evidence, confidence, and a decision implication before continuing.
3. **separate local vs central decisions.** Produce evidence, confidence, and a decision implication before continuing.
4. **manage shared resources.** Produce evidence, confidence, and a decision implication before continuing.
5. **detect drift.** Produce evidence, confidence, and a decision implication before continuing.
6. **replicate winning practices.** Produce evidence, confidence, and a decision implication before continuing.
...
Read [`references/domain-playbooks/operations.md`](../../../references/domain-playbooks/operations.md) when adapting this method to a specific industry, lifecycle stage, maturity level, or business model.
...
## KPIs
- `same-store performance` — baseline, target, actual, trend, confidence, and owner.
- `location variance` — baseline, target, actual, trend, confidence, and owner.
- `standards compliance` — baseline, target, actual, trend, confidence, and owner.
- `ramp time` — baseline, target, actual, trend, confidence, and owner.
...
## Example
**Founder intent:** "Use multi-location operations to improve our outcome without exceeding our cash, risk, or capacity limits."
