---
name: business-archetype-classifier
description: Classify how the company creates and delivers value so every downstream skill uses the right operating logic. Use when a founder or agent needs an evidence-backed business archetype classifier decision or operating plan.
metadata:
  domain: adaptation
  version: 1.0.0
  autonomy_ceiling: L2
  ...
  related_skills: "industry-operating-profile, dynamic-skill-routing"
  owned_by_agents: "adaptation-agent, adaptation-agent, business-analyst-agent"
---

# Skill: Business Archetype Classifier

## Purpose
Classify how the company creates and delivers value so every downstream skill uses the right operating logic. It operationalizes business archetype, industry, stage, geography, maturity, benchmark, and capability routing for companies of different sizes, models, industries, stages, and jurisdictions.

## When to Use
- The founder asks for a business archetype classifier diagnosis, decision, plan, or review.
...
## Analysis Framework
1. **Identify payer, user, unit, fulfillment, recurrence, asset intensity, regulation, channel, geography, and network effects.** Produce evidence, confidence, and a decision implication before continuing.
2. **assign primary and secondary archetypes.** Produce evidence, confidence, and a decision implication before continuing.
3. **Integrate.** Quantify cross-functional effects, dependencies, uncertainty, and opportunity cost in the Business Digital Twin.
4. **Decide.** Recommend the smallest action portfolio that clears hard constraints and maximizes risk-adjusted expected value.
5. **Learn.** Define leading indicators, review date, expected outcome, and conditions to stop, scale, reverse, or escalate.
...
## KPIs
- `classification confidence` — baseline, target, actual, trend, confidence, and owner.
- `downstream routing accuracy` — baseline, target, actual, trend, confidence, and owner.
- `override frequency` — baseline, target, actual, trend, confidence, and owner.
...
## Example
**Founder intent:** "Use business archetype classifier to improve our outcome without exceeding our cash, risk, or capacity limits."
